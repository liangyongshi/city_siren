...
import time
# import  lib.models.fourier as fourier_mapping

def fourier_mapping(coords,scale):
  """concate coords and fourier mapping
  """
  np.random.seed(1234)
  B = np.random.normal(size=(64, 3))*scale
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self, out_features=1, type='sine',in_features=3, 
                 mode='mlp', hidden_features=256, num_hidden_layers=3,pe=True,num_encoding_functions=6,**kwargs):
        super().__init__()
        self.mode = mode
        self.pe=pe
        self.num_encoding_functions=num_encoding_functions
        if self.pe:
           in_features=in_features*(2*num_encoding_functions+1) 
    ...
    def forward(self, model_input, params=None):
        ...
        if self.pe:
            coords=positional_encoding(coords_org,num_encoding_functions=self.num_encoding_functions)
        else:
            coords = coords_org
...
class semanticSIREN(MetaModule):
    def __init__(self, out_features=1, type='sine', in_features=3,
                 mode='mlp', hidden_features=256, num_hidden_layers=3, pemode='nerf',num_encoding_functions=6,**kwargs):
        super().__init__()
        self.mode = mode
        self.pemode=pemode
        self.num_encoding_functions=num_encoding_functions
        ...
        if self.pemode=='nerf':           
            in_features=in_features*(2*num_encoding_functions+1)            
            self.decoder_sdf = FCBlock(in_features=in_features, out_features=1, num_hidden_layers=num_hidden_layers,
                            hidden_features=hidden_features, outermost_linear=True, nonlinearity=type)
            
            self.decoder_label = FCBlock(in_features=in_features, out_features=19, num_hidden_layers=num_hidden_layers,
                            hidden_features=hidden_features, outermost_linear=True, nonlinearity=type)
       
        elif self.pemode=='fourier':
            in_features=  in_features+2*self.m         
            self.decoder_sdf = FCBlock(in_features=in_features, out_features=1, num_hidden_layers=num_hidden_layers,
                            hidden_features=hidden_features, outermost_linear=True, nonlinearity=type)            
            # label_in_features=3*(2*num_encoding_functions+1)
            self.decoder_label = FCBlock(in_features=in_features, out_features=19, num_hidden_layers=num_hidden_layers,
                            hidden_features=hidden_features, outermost_linear=True, nonlinearity=type)
       
        elif self.pemode=='xyz':  
    ...
    def forward(self, model_input, params=None):
        ...
        if self.pemode=='nerf':
            coords=positional_encoding(coords_org,num_encoding_functions=self.num_encoding_functions)
            sdfs = self.decoder_sdf(coords, get_subdict(params, 'decoder_sdf'))
            sdfs = self.dropout(sdfs)
            labels = self.decoder_label(coords, get_subdict(params, 'decoder_label'))
            # labels = self.dropout(labels)
        elif self.pemode=='fourier':
            scale = self.scale    
            coords = fourier_mapping(coords_org, scale)
            sdfs = self.decoder_sdf(coords, get_subdict(params, 'decoder_sdf'))
            sdfs = self.dropout(sdfs)
            # label_coords = positional_encoding(coords_org,num_encoding_functions=self.num_encoding_functions)
            # labels = self.decoder_label(label_coords, get_subdict(params, 'decoder_label'))  
            labels = self.decoder_label(coords, get_subdict(params, 'decoder_label'))         
        elif self.pemode=='xyz':   
            coords = coords_org
            sdfs = self.decoder_sdf(coords, get_subdict(params, 'decoder_sdf'))
        ...
        return {'model_in': coords_org, 'sdf_out': sdfs, 'label_out': labels}
