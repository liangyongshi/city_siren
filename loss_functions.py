...
def sdf(model_output, gt):
    ...
    # Exp      # Lapl
    # -----------------
    return {'sdf': torch.abs(sdf_constraint).mean() * 1e4,  # 1e4      # 3e3
            'inter': inter_constraint.mean() * 1e2,  # 1e2                   # 1e3
            'normal_constraint': normal_constraint.mean() * 1e2,  # 1e2
            'grad_constraint': grad_constraint.mean() * 1e1}  # 1e1      # 5e1

# inter = 3e3 for ReLU-PE
def sdf_label(model_output, gt):
    '''
       x: batch of input coordinates
       y: usually the output of the trial_soln function
       '''
    ...
    # label
    gt_labels = gt['labels']
    label_out=model_output['label_out']
    pred_labels = label_out.permute((0,2,1))
    
    # sum = np.zeros((20,))
    # for j in range(pred_labels.shape[1]):
    #     for i in range(pred_labels.shape[2]):   
    #         sum[j] += pred_labels[0][j][i]
    # print(sum) 
    
    label_loss = F.cross_entropy(pred_labels, gt_labels,ignore_index=255)
    ...
    return {'sdf': torch.abs(sdf_constraint).mean() * 1e4,  # 1e4      # 3e3
            ...
            'label_loss': label_loss.mean() * 1e1
            } 
