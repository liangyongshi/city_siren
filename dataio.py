...
class PointCloud(Dataset):
    def __init__(self, pointcloud_path, on_surface_points,fix_coordiante=False,keep_aspect_ratio=True,incremental_sampling=False,pre_size=0,first_frame=False,pre_sample_rate=0.5):
        super().__init__()
        
        self.fix_coordiante=fix_coordiante
        self.incremental_sampling=incremental_sampling
        self.pre_size=pre_size
        self.first_frame=first_frame
        self.pre_sample_rate=pre_sample_rate
        self.globalsample=False
        self.sample_car=True
        self.gauss_sample=True
        update_normals=True

        if update_normals:
            print("Loading point cloud")
            point_cloud = np.loadtxt(pointcloud_path,delimiter=' ')
            print("Finished loading point cloud")
            self.cars=np.asarray(np.where(point_cloud[:,3]==10))
            ...
            point_cloud = np.loadtxt(pointcloud_path, delimiter=' ')
            print("Finished loading point cloud")
            self.cars=np.asarray(np.where(point_cloud[:,6]==10))
            print("updating normals")
            coords = point_cloud[:, :3]  ##默认是深拷贝
            coords_temp = (point_cloud[:, :3]).copy() ##必须是浅拷贝，否则后面求最大最小值有误
            self.normals = point_cloud[:, 3:6]
            print("update normals finished")           
            self.labels=point_cloud[:, 6]
        ...
        if(self.fix_coordiante==True):
            ...
            # mean=np.array([[-0.45747816,88.48181298,-1.35900433]])  ### for kitti_07_label
            # coords -= mean
            # coord_max=237.346024
            # coord_min=-136.767064
            
            # mean_temp=np.mean(coords_temp, axis=0, keepdims=True)
            coords_temp-=mean
            coord_temp_max = np.amax(coords_temp, axis=0, keepdims=True)
            coord_temp_min = np.amin(coords_temp, axis=0, keepdims=True)           
            self.sample_up_limit=((coord_temp_max-coord_min)/(coord_max-coord_min)-0.5)*2
            self.sample_down_limit=((coord_temp_min-coord_min)/(coord_max-coord_min)-0.5)*2
            print("sample_down_limit:",self.sample_down_limit)
            print("sample_up_limit:",self.sample_up_limit)
        else:
    ...
    def __getitem__(self, idx):
        ...
        # Random coords
        if ((self.incremental_sampling==True) and (self.first_frame==False)):
            cars_point=9000
            if self.cars.shape[1]<cars_point:
                cars_point=self.cars.shape[1]
            pre_sample_size=int(self.pre_sample_rate*self.on_surface_points)
            rand_idcs1=np.random.choice(self.pre_size, size=pre_sample_size)
            sample_zone=np.arange(self.pre_size,point_cloud_size,1)
            rand_idcs2=np.random.choice(a=sample_zone,size=(self.on_surface_points-pre_sample_size-cars_point))
            rand_idcs3=np.random.choice(self.cars.squeeze(),size=cars_point)
            rand_idcs=np.concatenate((rand_idcs1,rand_idcs2),axis=0)
            rand_idcs=np.concatenate((rand_idcs,rand_idcs3),axis=0)
        elif self.sample_car==True:
            cars_point=9000
            if self.cars.shape[1]<cars_point:
                cars_point=self.cars.shape[1]
            rand_idcs1=np.random.choice(self.cars.squeeze(),size=cars_point)
            rand_idcs2=np.random.choice(point_cloud_size,size=(self.on_surface_points-cars_point))
            rand_idcs=np.concatenate((rand_idcs1,rand_idcs2),axis=0)
        else:
            if point_cloud_size<self.on_surface_points:
                self.on_surface_points=point_cloud_size
                
            rand_idcs = np.random.choice(point_cloud_size, size=self.on_surface_points)  ##generate an size-dimensional array whose upper limit is point_cloud_size 
        
        
        on_surface_coords = self.coords[rand_idcs, :]   ##random select points corresponding to rand_idcs          
        on_surface_normals = self.normals[rand_idcs, :]
        on_surface_labels = self.labels[rand_idcs]
        
        
        if self.globalsample:
            off_surface_coords = np.random.uniform(-1, 1, size=(off_surface_samples, 3))
        elif self.gauss_sample:
            off_points_near_truth = int(off_surface_samples*0.23)
            uniform_sample = int(off_surface_samples*0.5)
            rand_coords = np.random.choice(rand_idcs,size=off_points_near_truth)##小于off_surface_samples*0.5，因为off_surface_samples是on_surface_sample d的2倍
            
            off_surface_coords1 = self.coords[rand_coords, :] + np.random.normal(0.0,0.00005,size=(off_points_near_truth,3))
            # off_surface_coords1 = self.coords[rand_coords, :] + np.random.normal(np.array([0,0,0]),np.array([0.015,0.015,0.0005]),size=(off_points_near_truth,3))
            off_surface_coords1 = np.clip(off_surface_coords1,-1,1)
            
            off_surface_coords2 = np.random.uniform(self.sample_down_limit, self.sample_up_limit, size=(off_surface_samples-off_points_near_truth-uniform_sample, 3))
           
            off_surface_coords3 = np.random.uniform(-1, 1, size=(uniform_sample, 3)) 
            
            # off_surface_coords3_1=np.random.uniform(self.sample_up_limit,np.array((1,1,1)), size=(int(0.5*uniform_sample), 3)) 
            # off_surface_coords3_2=np.random.uniform(np.array((-1,-1,-1)),self.sample_down_limit, size=(uniform_sample-int(0.5*uniform_sample), 3))  
            # off_surface_coords3 = np.concatenate((off_surface_coords3_1,off_surface_coords3_2),axis=0)
                               
            off_surface_coords = np.concatenate((off_surface_coords1,off_surface_coords2),axis=0)   
            off_surface_coords = np.concatenate((off_surface_coords,off_surface_coords3),axis=0)       
        else:
            off_points_near_truth = int(off_surface_samples*0.5)
            off_surface_coords1 = np.random.uniform(self.sample_down_limit, self.sample_up_limit, size=(off_points_near_truth, 3))
            off_surface_coords2 = np.random.uniform(-1, 1, size=(off_surface_samples-off_points_near_truth, 3))
            off_surface_coords = np.concatenate((off_surface_coords1,off_surface_coords2),axis=0) 
        
            
 
        off_surface_normals = np.ones((off_surface_samples, 3)) * -1
        off_surface_labels = np.ones((off_surface_samples))*255
        ...
        labels = np.concatenate((on_surface_labels, off_surface_labels), axis=0).astype(np.int)
